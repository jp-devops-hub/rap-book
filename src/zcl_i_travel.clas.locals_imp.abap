      ...
      IF <ls_result>-CurrencyCode <> 'EUR'.
        ...
        INSERT VALUE #(
           ...
           %msg = new_message_with_text(
            ...
            text = 'Only Euro is accepted as currency'
           )
          ...
          IF sy-subrc = 0.
            ...
            APPEND VALUE #( travelid = ls_travel-travel_id %cid = ls_keys-%cid
                            %msg = new_message_with_text( severity = if_abap_behv_message=>severity-success
                                                          text = 'Travel' && | | && ls_travel-travel_id && | | && 'successfully updated' ) ) TO reported-z_i_travel.
          ELSE.
            APPEND VALUE #( travelid = ls_travel-travel_id %cid = ls_keys-%cid ) TO failed-z_i_travel.
            APPEND VALUE #( travelid = ls_travel-travel_id %cid = ls_keys-%cid
                            %msg = new_message_with_text( severity = if_abap_behv_message=>severity-error
                                                          text = 'Error for Travel ' && | | && ls_travel-travel_id ) ) TO reported-z_i_travel.
          ENDIF.
        ENDLOOP.
      ENDIF.
    ENDLOOP.
      ...
      IF sy-subrc = 0.
        ...
        APPEND VALUE #( travelid = ls_result-travelid %pid = ls_result-%pid
                   %msg = new_message_with_text( severity = if_abap_behv_message=>severity-information
                                                     text = |The Customer | && ls_result-CustomerId && | has | && lv_travels && | Travels| ) ) TO reported-z_i_travel.
      ENDIF.
    ENDLOOP.
